				...
				model.load(function(error) {
					...
					if (!error) {
						...
						for(var i = 0; i < data.rates.length; i++) {
							var template = '<div class="offer ' + 'offer' + i + ' col-sm-12 col-md-4 text-center"><div class="card__wrapper"><div class="card clearfix"><img class="card__photo" alt="highlight image"><h3 class="card__title"></h3><p class="card__text">from <span class="card__price"></span> per night</p><div class="col-xs-6 col-md-12"><div class="details"></div><div class="row"><a class="card__link">See more details <br></a></div></div><div class="col-xs-6 col-md-12"><div class="row"><a class="btn card__button">Book now</a></div></div></div></div></div>';
							var indexRow = parseInt(i / 3);
							if(i % 3 === 0) {
								$('.offers').append('<div class="row clearfix row' + indexRow + '">' + template + '</div>');
							} else {
								$('.offers .row' + indexRow).append(template);
							}
							var offer = data.rates[i];
							var title = jQuery.parseHTML(offer.rate.title);
							jQuery('.offer' + i + ' .card__title').text($(title).text());
							jQuery('.offer' + i + ' .card__price').text(offer.quotation.pricePerNight.toLocaleString() + ' ' + offer.quotation.currency);
							jQuery('.offer' + i + ' .card__photo').attr('src', offer.rate.image.url);
							jQuery('.offer' + i + ' .card__button').attr('onclick', offer.quotation.jsBookLink);
							jQuery('.offer' + i + ' .card__link').click(function(){
								$(this).empty();
								$(this).parent().parent().children('.details').append(offer.rate.html_description);
							})
						}
					}
				});
