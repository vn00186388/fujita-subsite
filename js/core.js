...
function openBookingEngine(url) {
    ...
    window.open(url, title, "toolbar=no,width=" + width + ",height=" + height + ",menubar=no,scrollbars=yes,resizable=yes,alwaysRaised=yes");
}
...
{
    mydoc.js__fbqs__form.langue.value=lang;
...
function ageschildren()
{
    var age = '';
    var obj1 = '';
    var n = document.js__fbqs__form.enfantresa.options[document.js__fbqs__form.enfantresa.selectedIndex].value;
    ...
    document.js__fbqs__form.agesval.value = age;
}
...
'use strict';
var fujitaQsDatepicker = (function(window){

    var format      = '';
    var beFormat    = '';
    var lang        = 'en';

    var months      = ( window.innerWidth <= 991 || window.screen.width <= 991 )? 1 : 3;
    ...
    // Assign date text format depend on site language.
    switch (lang) {
        case 'en':
            format      = 'mm/dd/yy';
            beFormat    = 'MM/DD/YYYY';
    ...
    var $headerQs = $('.js_header__qs');
    var $checkinInput = $headerQs.find('.js_qs__checkin');
    var $checkoutInput = $headerQs.find('.js_qs__checkout');

    var qsUpdateSelect = function(name, date ) {

        $('.js__fbqs__' + name ).val( date ).change();

        date = $('.js__fbqs__' + name).val();
        var dateArr = date.split("/");

        var parent = $('.js__fbqs__' + name).parent();
        parent.find("option[selected=selected]").removeAttr('selected');
        if(lang  === 'en') {
            parent.find(".js__fbqs__" + name + "-month").val(dateArr[0]);
            parent.find(".js__fbqs__" + name + "-day").val(dateArr[1]);
            parent.find(".js__fbqs__" + name + "-year").val(dateArr[2]);
        } else {
            parent.find(".js__fbqs__" + name + "-month").val(dateArr[1]);
            parent.find(".js__fbqs__" + name + "-day").val(dateArr[2]);
            parent.find(".js__fbqs__" + name + "-year").val(dateArr[0]);
        }
    }

    // var datepickerSetup = function() {
    //     var months      = ( window.innerWidth <= 991 || window.screen.width <= 991 )? 1 : 2;
    //
    //     // Setup datepickers.
    //
    ...
    var init = function() {
        ...
        $checkinInput.datepicker({
            onSelect: function( date, obj ) {
                ...
                $checkoutInput.datepicker( 'option', { minDate: new Date(next_date) });
...
'use strict';
var fujitaQsHotelBooking = (function(window){
    var $header = $('.js_header');
    var $bookingForm = $('.js_qs-form');

    // $header.on('setHotelData', function(e, data) {
    //   var $headerQsSites = $('.js_qs__sites');
    //   console.log($headerQsSites.find('option'));
    //   $headerQsSites.find('option[value="' +data.hotel_fb_connectname+ '"]').attr('selected', 'selected');
    //
    //   console.log($headerQsSites.find('option[value="' +data.hotel_fb_connectname+ '"]'));
    //   $headerQsSites.val(data.hotel_fb_connectname);
    // });

    var setInputCluster = function(cluster) {
        var $clusterInputHidden = $bookingForm.find('.js_cluster');
        var $btnViewBooking = $bookingForm.find('.js_qs__btn-view');

        $clusterInputHidden.val(cluster);
        $btnViewBooking.attr('href', $btnViewBooking.attr('href').replace(/Hotelnames=[^&]*/, 'Hotelnames=' + cluster));
    };

    var setQsSites = function(data) {
        var $qsSites = $bookingForm.find('.js_qs__sites');

        $qsSites.empty();

        for (var hotel in data) {
            if (data[hotel].hotel_fb_connectname) {
                $qsSites.append('<option value="' + data[hotel].hotel_fb_connectname + '" >' + data[hotel].post_title + '</option>');
            }
        }

        setInputCluster($qsSites.val());

        $('.qs__viewport').text($('.js_qs__sites option:selected').text());
        
        $qsSites.on('change', function() {
            setInputCluster(this.value);
            $('.qs__viewport').text($("option:selected", this).text());
        });
    };
...
$(document).ready(function() {
    ...
    fujita.init();
    fujitaQsDatepicker.init();
    fujita.pageSlider('.js_slide-show', fujita.optionSlider() ,true);
    fjtss_get_json(window.allHotelsJsonUrl, fujitaQsHotelBooking.init);
    fjtss_get_json(subsite_hotel_rest_url, fujita.initHeaderFooterInfo);
    fjtss_get_json('http://fujita-group.wsdasia-sg-1.wp-ha.fastbooking.com/wp-json/fujita-group/v1/hotels/order', fujita.initHotelList);
    if (!$('html').hasClass('mobile'))
        $('head').append('<script src="//load.sumome.com/" data-sumo-site-id="ee8366b10441ec8833b205eae9f66e4df918bd3ac70b16e8277e0fe63842165c" async="async"></script>');
